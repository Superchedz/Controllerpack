...
# we need to handle a hysteresis curve to avoid turning on and off repeatedly.
  ...
  if curr_zone_state == "ON":
    if curr_zone_temp >= Temp_Upper:
      if DebugMode == "Y":
        print "Turning zone off as temp was high"
    ...
    if curr_zone_temp <= Temp_Lower:
      if DebugMode == "Y":
        print "Turning zone on as temp is low"
...
while sendcounter < 10:
  sendcounter += 1				  
  send_alert('Alert: Heating Control1 - STARTUP','System starting')
  if sendok:
    sendcounter = 11;  
...
#  it doesnt matter if we find more than 1...put the heating on.
...
#######  call function to turn on heating, set the status in DB
#     but first check if the zone has no temp, it could be hot water, a light or a pump...HomeAutomation9000 POWER!!! 
